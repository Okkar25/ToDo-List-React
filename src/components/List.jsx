...
const List = ({ job, id, isDone, deleteTask, doneTask, updateTask }) => {
  ...
  const handleCheck = () => {
    setCheck(!check);
    doneTask(id); // status done task update 0/3
  };
  ...
  // update task when cursor placed outside input
        ...
        {edit ? (
          <div className="">
            <input
              ...
              className={`border border-gray-400 text-sm py-1 px-2 w-[280px] focus-visible:outline-blue-500`}
            />
          </div>
        ) : (
